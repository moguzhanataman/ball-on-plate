	...
	//	cout << "X -> " << x << " and Y -> " << y << endl;
		...
		/*	
			...
			if(child == 0){
				...
				close(fd[1]);
				pthread_create(&thread, NULL, pipe_thread, fd);		
			}*/
		}
...
void* vision(void* arg){
	try{
		...
		long lastSaved = tp.tv_sec * 1000 + tp.tv_usec / 1000;
		int sentPeriod = 150;
		int savePeriod = 100;
        ...
        do{
            ...
            ***/
            ...
            for (unsigned int i = 0; i < Markers.size(); i++) {
				...
				if(cur - lastSaved >= savePeriod){
					
					lastSaved = cur;
					
					que.push(map_float(Markers[i].getCenter().x, 45.0, 595.0, 800.0, 200.0));
					que.push(map_float(Markers[i].getCenter().y, 65.0, 425.0, 250.0, 775.0));
...
void* pipe_thread(void* arg){
	...
	for(;;){
		...
		while(n<4)
			...
			n+=read(fd,(void*)((char*)&f+n), 1);
		
		n=0;
		
		while(n<4)
			
			n+=read(fd,(void*)((char*)&f1+n), 1);
